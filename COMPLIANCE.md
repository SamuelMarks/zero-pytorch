...
The `ml-framework-snapshots` tool verifies that `zero-pytorch` accurately replicates the public API signatures, parameters, and defaults of the real `torch` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check torch ~/repos/zero-pytorch/src \
    --reference-prefix torch \
    --target-prefix zero_torch
```

**What it does:**
- It statically analyzes all Python modules in `zero-pytorch/src`.
- It maps the target namespace (`zero_torch.*`) back to the reference framework (`torch.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-pytorch/src
```
